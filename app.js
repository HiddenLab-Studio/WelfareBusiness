...
function create() {
    let game = this
    var sprite = this.add.sprite(config.width * 0.5, 300, 'button_play').setInteractive();
...
function update() {
    ...
    if (open == false) {
        spriteSettings.on('pointerdown', function (pointer) {
            ...
            volume = game.add.image(config.width * 0.5 -125, 200, 'volume').setScale(0.15) 
            music = game.sound.add('accueil')

            music.loop = true
            music.play()

            mute = game.add.sprite(config.width * 0.5 - 75, 200, 'mute').setScale(0.04)
