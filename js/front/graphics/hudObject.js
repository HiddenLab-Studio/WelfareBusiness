class hudObject {
    ...
    initializeUI() {
        let hud = this;
        let maps = new map(this.phaser, this.config)
        //Bouton play
        this.playbtn = this.phaser.add.sprite(this.config.width * 0.5, 300, 'button_play').setInteractive();
        ...
        //Bouton de settings
        this.settingsbtn = this.phaser.add.sprite(config.width - this.arbitraryUnit - 30, config.height - this.arbitraryUnit - 30, 'button_settings').setOrigin(0, 0).setInteractive().setScale(0.5);
    ...
    createHud() {
        ...
        //Barre d'argent
        this.moneyInfoBar = this.phaser.add.image(0, 0, 'argent_hud').setOrigin(0, 0).setScale(0.5);
        this.moneyString = "44 444 $";
        this.moneyText = this.phaser.add.text(60, 30, this.moneyString, { font: "18px Arial", fill: "#FFFFFF" });

        //Barre de bonheur
        this.happinessInfoBar = this.phaser.add.image(175, 0, 'bonheur_hud').setOrigin(0, 0).setScale(0.5);
        this.happinessString = "Valeur bonheur";
        this.happinessText = this.phaser.add.text(2 * (this.arbitraryUnit + 20) + 35, 30, this.happinessString, { font: "14px Arial", fill: "#FFFFFF" });

        //Barre de la date
        this.dateInfoBar = this.phaser.add.image(4 * this.arbitraryUnit + 3 * 20, 0, 'date_hud').setOrigin(0, 0).setScale(0.5);
        this.dateString = "04/09/02";
        this.dateText = this.phaser.add.text(4 * this.arbitraryUnit + 3 * 20 + 60, 30, this.dateString, { font: "14px Arial", fill: "#ffffff" });

        //Icones gestion du temps
        this.pausebtn = this.phaser.add.image(500, 20, 'pause').setOrigin(0, 0).setScale(0.25).setInteractive();
        ...
        this.playbtn = this.phaser.add.image(540, 20, 'play').setOrigin(0, 0).setScale(0.25).setInteractive();
        ...
        this.avancerapidebtn = this.phaser.add.image(580, 20, 'avance_rapide').setOrigin(0, 0).setScale(0.25);


        //Bouton du shop
        //this.shopStyle = this.phaser.make.graphics().fillStyle(0xffff00).fillRect(0, 0, this.arbitraryUnit, this.arbitraryUnit);
        this.shopbtn = this.phaser.add.sprite(30, config.height - this.arbitraryUnit - 30, 'button_shop').setOrigin(0, 0).setInteractive().setScale(0.5);
        ...
        this.progressbar = displayProgressBar(this.phaser, 100, this.config.width);
