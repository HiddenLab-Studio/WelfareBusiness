...
function update() {
    ...
    if (start == 0) {
        ...
        sprite.on('pointerdown', function (pointer) {
            ...
            //Fonction d'affichage de la barre de progression en fonction du poucentage de progression
            function displayProgressBar(game, percent) {
                let progressbarStyle = game.make.graphics().fillStyle(0x00ff00).fillRect(0, 0, percent * config.width, 10);
                progressbarStyle.generateTexture('progressbar', percent * config.width, 10);

                let progressbar = game.add.image(0, 0, 'progressbar').setOrigin(0, 0);
                progressbar.setDisplaySize(percent, 10);

                return progressbar;
            }


            let progressbar = displayProgressBar(this, 50);
